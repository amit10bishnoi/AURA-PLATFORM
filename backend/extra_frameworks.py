...
EXTRA_FRAMEWORKS = {
    "RBI": [
        ...
        {"id":"RBI-5",  "name":"VAPT by CERT-In empanelled auditors",            "weight":10, "fields":[("vulnerabilities","lte",10)]},
        {"id":"RBI-6",  "name":"Data encryption in transit and at rest",         "weight":12, "fields":[("has_mfa","bool",True)]},
        {"id":"RBI-7",  "name":"Incident response and RBI reporting (2-6 hrs)",  "weight":12, "fields":[("has_irp","bool",True)]},
        {"id":"RBI-8",  "name":"Customer data localisation in India",            "weight":10, "fields":[("has_irp","bool",True)]},
    ...
    "DPDP": [
        {"id":"DPDP-1", "name":"Obtain free and informed consent from users",    "weight":15, "fields":[("has_mfa","bool",True)]},
        {"id":"DPDP-2", "name":"Notice to data principals in plain language",    "weight":10, "fields":[("has_irp","bool",True)]},
        {"id":"DPDP-3", "name":"Data minimisation — collect only what is needed","weight":12, "fields":[("vulnerabilities","lte",15)]},
        {"id":"DPDP-4", "name":"Data accuracy and completeness",                 "weight":8,  "fields":[("patch_days","lte",60)]},
        {"id":"DPDP-5", "name":"Erase data when purpose fulfilled",              "weight":10, "fields":[("has_irp","bool",True)]},
        {"id":"DPDP-6", "name":"Personal data breach notification to DPBI",      "weight":15, "fields":[("has_irp","bool",True)]},
        {"id":"DPDP-7", "name":"Right of access — provide data within 48 hours", "weight":10, "fields":[("has_mfa","bool",True)]},
        {"id":"DPDP-8", "name":"Right to correction and erasure",                "weight":10, "fields":[("has_irp","bool",True)]},
        {"id":"DPDP-9", "name":"Cross-border data transfer restrictions",        "weight":5,  "fields":[("has_irp","bool",True)]},
...
def score_extra_framework(key: str, assessment) -> dict:
    ...
    for control in controls:
        ...
        for (field, op, expected) in fields:
            ...
            if val is None:
                val = assessment.get(field) if isinstance(assessment, dict) else None
    ...
    score = round((earned / total_weight) * 100, 2) if total_weight > 0 else 0.0
    return {
        "framework": key,
