...
const[messages,setMessages]=useState([{id:0,role:"assistant",content:"Hi! I'm AURA AI. I can analyze your compliance posture, explain risk scores, suggest remediations, and help you prioritize what to fix. What would you like to know?",category:"General",timestamp:new Date().toISOString()}]);
...
useEffect(()=>{
fetch(`${API}/api/ai/suggestions?tenant_id=${tenantId||"demo"}`,{headers:{Authorization:`Bearer ${token}`}}).then(r=>r.json()).then(d=>setSuggestions(d.suggestions||[])).catch(()=>{});
fetch(`${API}/api/ai/recommendations?tenant_id=${tenantId||"demo"}`,{headers:{Authorization:`Bearer ${token}`}}).then(r=>r.json()).then(d=>setRecommendations(d.recommendations||[])).catch(()=>{});
},[token,tenantId]);
...
{m.category&&<div style={{fontSize:10,fontWeight:600,color:CAT_COLORS[m.category]||"#94A3B8",marginBottom:4,textTransform:"uppercase",letterSpacing:".05em"}}>{m.category}{m.confidence&&<span style={{color:"#334155",marginLeft:6}}>· {m.confidence}% confidence</span>}</div>}
<div style={{background:m.role==="user"?"rgba(99,102,241,.15)":"#0F172A",border:`1px solid ${m.role==="user"?"rgba(99,102,241,.3)":"#1E293B"}`,borderRadius:m.role==="user"?"12px 12px 2px 12px":"12px 12px 12px 2px",padding:"10px 14px",color:"#CBD5E1",fontSize:13,lineHeight:1.6}}>{m.content}</div>
