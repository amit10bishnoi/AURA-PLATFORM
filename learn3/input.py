def get_inputs():
    ...
    mfa_coverage = int(input("MFA coverage percentage (0-100): ")) if has_mfa else 0
    patch_days = int(input("How many days between patches: "))
    training = int(input("Staff security training completion % (0-100): "))
    has_irp = input("Do you have an incident response plan? (yes/no): ").lower() == "yes"
    vulns = int(input("Number of open vulnerabilities: "))
